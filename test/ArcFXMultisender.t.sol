...
contract ArcFXMultisenderTest is Test {
    ...
    function testFuzz_FeeCalculation_IsAlwaysExact(uint256 total) public pure {
        ...
        assertEq(expectedFee + expectedNet, total, "fee + net must equal total -- no token dust");
        ...
        assertEq(expectedFee, (total * FEE_BPS) / BPS_DENOM, "fee must be exactly FEE_BPS/BPS_DENOM of total");

        // Fee is always strictly less than the gross
        assertLt(expectedFee, total, "fee must be less than the gross amount");
    }
    ...
    function testFuzz_QuoteTotalMatchesInternalCalc(uint8 n, uint64 amountEach) public view {
        ...
        (uint256 qTotal, uint256 qFee, uint256 qWithFee) = multisender.quoteTotal(amounts, true);
    ...
    function testFuzz_Treasury_ReceivesExactFee(uint8 n, uint64 amountEach) public {
        ...
        assertEq(token.balanceOf(treasury) - tBefore, fee, "Treasury must receive exactly the 0.10% fee");
    }
    ...
    function testFuzz_FreeTier_TreasuryReceivesNoFee(uint8 n, uint64 amountEach) public {
        ...
        assertEq(token.balanceOf(treasury), tBefore, "Free tier must not send any fee to treasury");
    }
    ...
    function testFuzz_Contract_HoldsZeroResidualBalance(uint8 n, uint64 amountEach) public {
        ...
        assertEq(
            token.balanceOf(address(multisender)), 0, "Multisender must not retain any tokens after a complete send"
        );
    }
    ...
    function testFuzz_EachRecipient_ReceivesExactAmount(uint8 n, uint64 amountEach) public {
        ...
        for (uint256 i = 0; i < count; i++) {
            assertEq(token.balanceOf(recipients[i]), amt, "Recipient received wrong token amount");
        }
    }
    ...
    function testFuzz_Sender_BalanceDecreasedByPull(uint8 n, uint64 amountEach) public {
        ...
        assertEq(balBefore - token.balanceOf(sender), pull, "Sender balance must decrease by exactly total + fee");
    }
    ...
    function testFuzz_Revert_ZeroAddressRecipient_AnyPosition(uint8 badIdx) public {
        ...
        uint256[] memory amounts = new uint256[](n);
        for (uint256 i = 0; i < n; i++) {
            recipients[i] = (i == zeroAt) ? address(0) : makeAddr(string(abi.encodePacked("r", vm.toString(i))));
            amounts[i] = 1e6;
        }
    ...
    function test_Unit_MinimumAmount_OneUnit() public {
        ...
        assertEq(token.balanceOf(recipients[0]), 1, "Recipient must receive 1 unit when fee rounds to zero");
    }
