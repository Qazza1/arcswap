...
export function mountTrade(root: HTMLElement): void {
  ...
  localBridgeProof?.addEventListener("click", async () => {
    ...
    try {
      ...
      console.info("ArcFX local bridge proof result", { state: result.state, errorState: result.errorState, sourceTxHashes: result.sourceTxHashes, destinationTxHashes: result.destinationTxHashes, attestationState: result.attestationState, destinationState: result.destinationState });
