...
async function render(): Promise<void> {
  ...
  const ready = arcfxWallet.connected && arcfxWallet.chainId?.toLowerCase() === "0x13b2" && arcfxApi.hasReceivablesOwnerSession();
  if (!ready || !wallet) {
  ...
  const current = () => version === renderVersion && arcfxWallet.address?.toLowerCase() === wallet && arcfxWallet.chainId?.toLowerCase() === "0x13b2" && arcfxApi.hasReceivablesOwnerSession();
